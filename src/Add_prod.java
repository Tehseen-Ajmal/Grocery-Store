 ...
 * @author hp
 */
public class Add_prod extends javax.swing.JPanel {
    ...
    private static int getNextPrimaryKey(Connection connection) throws SQLException {
        ...
        String maxPrimaryKeyQuery = "SELECT MAX(CAST(SUBSTRING(ProductID, 2, LEN(ProductID) - 1) AS INT)) AS MaxID FROM Products";
    ...
    private void button1ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, pass)) {
            ...
            String sql = "INSERT INTO Products (ProductID,ProdName, ProdDescription, UnitPrice, CostPrice, QuantityInstock, CategoryID, SupplierID, Discontinued) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?);";
            try (PreparedStatement pstmt = con.prepareStatement(sql)) {
                pstmt.setString(1,"P"+nextPrimaryKey);
                pstmt.setString(2,email_person1.getText() );
                pstmt.setString(3, desc.getText());
                pstmt.setFloat(4, (float) Double.parseDouble(price.getText()));
                pstmt.setFloat(5, Float.parseFloat(cost.getText()));
                pstmt.setInt(6, Integer.parseInt(quantity.getText()));
                pstmt.setString(7, (String) jComboBox1.getSelectedItem());
                pstmt.setString(8, (String) jComboBox2.getSelectedItem());
                if(((String)jComboBox3.getSelectedItem()).equals("yes")) {
                    pstmt.setInt(9,0);
                }else{
                    pstmt.setInt(9,1);
                }
                ...
                if (rowsInserted > 0) {
                    ...
                    JOptionPane.showMessageDialog(jPanel4, "Product Added...\nPID : P"+nextPrimaryKey, "Success", JOptionPane.INFORMATION_MESSAGE);
                } else {
